...
<div style="margin-bottom: 30px; display: flex; gap: 15px; flex-wrap: wrap;">
    ...
    <a href="{{ route('admin.services.overview', $slug) }}" class="btn-gold" style="text-decoration: none; display: inline-flex; align-items: center; gap: 8px;">
...
<div class="admin-card">
    <form action="{{ route('admin.services.overview.update', $slug) }}" method="POST" enctype="multipart/form-data" class="admin-form">
        @csrf

        <div class="form-group">
            <label for="label">Small Label (Eyebrow)</label>
            <input type="text" name="label" id="label" class="form-control" value="{{ old('label', $overviewSettings['label'] ?? '') }}" required>
            @error('label') <span class="error" style="color: #F44336; font-size: 13px;">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label for="description">Description (Paragraphs)</label>
            <textarea name="description" id="description" class="form-control" rows="6" required>{{ old('description', $overviewSettings['description'] ?? '') }}</textarea>
            <small style="color: var(--muted); font-size: 13px; display: block; margin-top: 6px;">Use new lines to create separate paragraphs.</small>
            @error('description') <span class="error" style="color: #F44336; font-size: 13px;">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label for="sub_heading">Bullets Sub-heading</label>
            <input type="text" name="sub_heading" id="sub_heading" class="form-control" value="{{ old('sub_heading', $overviewSettings['sub_heading'] ?? '') }}" required>
            @error('sub_heading') <span class="error" style="color: #F44336; font-size: 13px;">{{ $message }}</span> @enderror
        </div>
