...
<div style="margin-bottom: 30px; display: flex; gap: 15px; flex-wrap: wrap;">
    ...
    <a href="{{ route('admin.services.benefits.index', $slug) }}" class="btn-gold" style="text-decoration: none; display: inline-flex; align-items: center; gap: 8px;">
...
<!-- Benefits Header Settings -->
<div class="admin-card" style="margin-bottom: 30px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2 style="font-size: 20px; font-weight: 600;">Section Header</h2>
    </div>

    <form action="{{ route('admin.services.benefits.header', $slug) }}" method="POST" class="admin-form">
        @csrf
        ...
        <div class="form-group">
            ...
            <textarea name="description" id="description" class="form-control" rows="3" required>{{ old('description', $header['description'] ?? '') }}</textarea>
...
<!-- Benefits Cards Settings -->
<div class="admin-card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2 style="font-size: 20px; font-weight: 600;">Benefit Cards</h2>
        <a href="{{ route('admin.services.benefits.create', $slug) }}" class="btn-gold btn-sm" style="text-decoration: none; padding: 8px 16px;">+ Add Card</a>
    </div>
                ...
                <tr>
                    ...
                    <td>
                        <div style="display: flex; gap: 10px;">
                            <a href="{{ route('admin.services.benefits.edit', [$slug, $card['id']]) }}" class="btn-icon" title="Edit">
                                <i data-feather="edit-2"></i>
                            </a>
                            <form action="{{ route('admin.services.benefits.destroy', [$slug, $card['id']]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this card?');">
                                @csrf
                                @method('DELETE')
    ...
    @else
    <p style="color: var(--muted); padding: 20px 0; text-align: center;">No benefit cards added yet.</p>
    @endif
</div>
