...
<div class="admin-card">
    <form action="{{ route('admin.faqs.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="category" id="category" class="form-control" value="{{ old('category') }}" placeholder="e.g. Pricing, Process, Timelines">
        ...
        <div class="form-group" style="margin-top: 30px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 10px;">
                <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                    <input type="checkbox" name="pages[]" value="services" {{ (is_array(old('pages')) && in_array('services', old('pages'))) ? 'checked' : '' }}>
                    Services Page
                </label>
                <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                    <input type="checkbox" name="pages[]" value="case-study" {{ (is_array(old('pages')) && in_array('case-study', old('pages'))) ? 'checked' : '' }}>
                    Case Study Page
                </label>
                <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                    <input type="checkbox" name="pages[]" value="portfolio" {{ (is_array(old('pages')) && in_array('portfolio', old('pages'))) ? 'checked' : '' }}>
                    Portfolio Page
                </label>
            </div>
        ...
        <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.05);">
            <button type="submit" class="btn btn-gold">Save FAQ</button>
        </div>
    </form>
</div>
